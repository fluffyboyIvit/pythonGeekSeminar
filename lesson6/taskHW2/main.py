...
def FillList(length):
    ...
    for i in range(length):
        ...
        list1.append(a)
    return list1
def winnerFunction(list1,list2,length):
    result = []
    for i in range(length):
        if list1[i] > list2[i]:
...
lengthInput = 20
firstTeamList = FillList(lengthInput)
print(f"Первая команда: {firstTeamList}")
secondTeamList = FillList(lengthInput)
print(f"Вторая команда: {secondTeamList}")
winnerList = winnerFunction(firstTeamList,secondTeamList,lengthInput)
print(f"победители: {winnerList}")
